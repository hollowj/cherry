...
func Repl(opts ...Option) {
	...
	shell := ishell.New()
	logger = NewCliLogger(shell)
	configure(shell)

	cherryLogger.Info("Nano REPL Client")

	registerHelp(shell)

	registerInfo(shell)
	registerHttpLogin(shell)
	registerDirectLogin(shell)
	registerHttpRegister(shell)
...
func registerHttpLogin(shell *ishell.Shell) {
	shell.AddCmd(&ishell.Cmd{
		Name:    "httpLogin",
		Aliases: []string{"hl"},
		Func: func(c *ishell.Context) {
			if len(c.Args) < 3 {
				c.Err(errors.New("httpLogin requires at least 3 arguments"))
				return
			}
			token, err := httpLogin(c.Args[0], c.Args[1], c.Args[2])
			if err != nil {
				c.Err(err)
			}
			c.Println(fmt.Sprintf("Token:%s", token))
		},
		Help: "login http with account & password",
	})
}

func registerHttpRegister(shell *ishell.Shell) {
	shell.AddCmd(&ishell.Cmd{
		Name:    "httpRegister",
		Aliases: []string{"hr"},
		Func: func(c *ishell.Context) {
			if len(c.Args) < 3 {
				c.Err(errors.New("httpRegister requires at least 3 arguments"))
				return
			}
			err := httpRegister(c.Args[0], c.Args[1], c.Args[2])
			if err != nil {
				c.Err(err)
			}
		},
...
// 直接登录 http login, tcp req auth
func registerDirectLogin(shell *ishell.Shell) {
	shell.AddCmd(&ishell.Cmd{
		Name:    "directLogin",
		Aliases: []string{"DLogin"},
		Func: func(c *ishell.Context) {
			if len(c.Args) < 6 {
				c.Err(errors.New("http_login requires at least 6 arguments"))
				return
			}
			err := directLogin(c.Args)
			if err != nil {
				c.Err(err)
			}
		},
